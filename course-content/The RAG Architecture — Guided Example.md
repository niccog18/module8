# The RAG Architecture — Guided Example

**Module 8 — RAG Intro & Docker Deployment**

`[VIDEO PLACEHOLDER: 8 min — "RAG Architecture Walkthrough: trace a real question through each stage of the pipeline. Show the query embedding, ChromaDB retrieval, prompt assembly, and LLM generation happening step by step with visible intermediate outputs."]`

Let’s trace a real question through the complete RAG pipeline, seeing the output at each stage. This gives you a concrete understanding of what happens at every step.
...
A step-by-step trace showing: the user’s question, the 3 most relevant chunks retrieved from ChromaDB (with the JWT auth document ranked first), the assembled prompt with system instructions and context, and a mock response that references the source.

The mock response simulates what a real LLM would generate. In Lesson 2, you’ll install Ollama and replace the mock with a real local model. In Lesson 3, you’ll build the full pipeline.

The key insight: the quality of the RAG response depends on **two things** — the quality of retrieval (finding the right documents) and the quality of the prompt (telling the model how to use them). You’ve already mastered retrieval in Module 7. Now you’re adding generation.

`[DIAGRAM PLACEHOLDER: Annotated screenshot of the script output showing each step labeled: Query → Retrieve → Build Prompt → Generate → Answer with Source]`